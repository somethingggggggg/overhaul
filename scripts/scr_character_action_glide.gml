...
if (Action = ActionDie){exit}
   ...
   // Grab onto a wall:
      if(Action = ActionGlide){
         if(((scr_character_calculate_angle(x, y, Angle) == 0) && XSpeed >= 0 && scr_character_collision_right(x, y, 0, spr_mask_big) && scr_character_collision_right(x, y, 0, spr_mask_climb_ledge)) && !scr_character_collision_right_object(x, y, 0, spr_mask_big, obj_monitor) && !scr_character_collision_right_object(x, y, 0, spr_mask_big, par_spike) && !scr_character_collision_right_object(x, y, 0, spr_mask_big, par_spring) && !scr_character_collision_top(x, y, 0, spr_mask_main))
         ||(((scr_character_calculate_angle(x, y, Angle) == 0) && XSpeed <= 0 && scr_character_collision_left(x, y, 0, spr_mask_big) &&  scr_character_collision_left(x, y, 0, spr_mask_climb_ledge)) &&  !scr_character_collision_left_object(x, y, 0, spr_mask_big, obj_monitor) &&  !scr_character_collision_left_object(x, y, 0, spr_mask_big, par_spike) && !scr_character_collision_left_object(x, y, 0, spr_mask_big, par_spring)  && !scr_character_collision_top(x, y, 0, spr_mask_main))
      
         {
               ...
               // If we're hyper, do a earthquake and destroy near enemies:
                  if(CharacterState == CharacterHyper){
                     ...
                     with(obj_camera){
                          ...
                          Camera_ShakeForce    = 5;
                          Camera_ShakeSound    = SFX.snd_object_crumble;
                     }
               ...
               // Play Sound:
                  PlaySound(SFX.snd_character_hang, global.SFXVolume, 1, 1);
