...
if (Action = ActionDie){exit}
   ...
   // Sounds:
      if(Action = ActionFly && !Underwater){
         if(CheckSound(SFX.snd_character_flying) == false){
            LoopSound(SFX.snd_character_flying, global.SFXVolume, 1);
         }
         if(CheckSound(SFX.snd_character_flying_fall) == true){
            StopSound(SFX.snd_character_flying_fall);
         }
      }
      if(Action = ActionFlydrop && !Underwater){
         if(CheckSound(SFX.snd_character_flying_fall) == false){
            LoopSound(SFX.snd_character_flying_fall, global.SFXVolume, 1);
         }
         if(CheckSound(SFX.snd_character_flying) == true){
