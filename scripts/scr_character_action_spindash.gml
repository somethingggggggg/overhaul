...
if (Action = ActionDie){exit}
         ...
         // If the character doesn't hold down anymore, roll:
            if(!KeyDown && Ground){
                ...
                // Stop the charging sound and play the dashing sound:
                   if(!CheckSound(SFX.snd_character_spindash_release)){
                       StopSound(SFX.snd_character_spindash_charge);
         ...
         // If the character presses (action) again, add power to the spindash accumulator.
            if(KeyAction_Pressed){
               ...
               // Play Charging sound:
                  ...
                  PlaySound(SFX.snd_character_spindash_charge, global.SFXVolume, 1+SpindashPitch,1); 
               ...
               // Reset pitch if the charging sound stopped:  
                  if(!CheckSound(SFX.snd_character_spindash_charge)){
                      SpindashPitch = -0.10;
                  }
  ...
  // Perform the Spindash:
     if(Action = ActionCrouch && KeyAction_Pressed){
        ...
        PlaySound(SFX.snd_character_spindash_charge, global.SFXVolume, 1,1)  // Play charging sound.
     }
  ...
  // If the character is running, on the ground and presses down, roll!
     if(Action = ActionNormal && Ground && abs(XSpeed) >= 1.03125 && KeyDown && !KeyLeft && !KeyRight){
        ...
        // Stop the Peelout sound:
           if(CheckSound(SFX.snd_character_peelout_release) == 1){
              StopSound(SFX.snd_character_peelout_release)
           }
