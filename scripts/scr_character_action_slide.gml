...
if (Action = ActionDie){exit}
   if(Action = ActionSlide){
      ...
      // Slide sound:
         if(CheckSound(SFX.snd_character_slide) == false){
            LoopSound(SFX.snd_character_slide, global.SFXVolume, 1);
         }
...
// Stop Sound.   
   if(Action != ActionSlide && CheckSound(SFX.snd_character_slide) == true){
      StopSound(SFX.snd_character_slide);
   }
