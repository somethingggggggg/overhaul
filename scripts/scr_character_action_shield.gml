         ...
         if(!Ground && Animation = "ROLL" && KeyAction_Pressed){
            ...
            // Flame Shield:
               if(Shield == ShieldFlames){
                  ...
                  // Play Shield sound:  
                     PlaySound(SFX.snd_shield_use_flame, global.SFXVolume, 1, true); 
            ...
            // Electro Shield:
               if(Shield == ShieldElectricity){  
                  ...
                  // Play Shield sound:  
                     PlaySound(SFX.snd_shield_use_electricity, global.SFXVolume, 1, true)
         ...
         // Bounce when we land on ground with the bubble shield!
            if(Shield == ShieldBubble){                   
               if(ShieldChild.Shield_State == "bounce" && ShieldUsable == false) { 
                  ...
                  if(Ground){
                        YSpeed = -(dsin(RelativeAngle) * GSpeed) - (dcos(RelativeAngle) * 3.75);    
                        ShieldAttack = 1;                         
                     }
                     XSpeed =  (cos(degtorad(RelativeAngle)) * XSpeed) + (sin(degtorad(RelativeAngle)) * YSpeed );      
                        }                   
                  ...
                  AnimationSpeed = 0.25;
                  ShieldUsable = true;         
                  PlaySound(SFX.snd_shield_use_bubble, global.SFXVolume, 1, true)
               }else
