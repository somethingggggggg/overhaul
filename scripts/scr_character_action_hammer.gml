...
if Action = ActionRail || Action = ActionSlipping{exit}
       ...
       if(AmyHammerAttack == 0){
          ...
          // Hammer Air Dash:
             if(Ground == false && Action == ActionJump && KeySpecial_Pressed){
                ...
                PlaySound(SFX.snd_shield_use_insta, global.SFXVolume, 1.2, true);
             }
         ...
         // End hammer punch:
            if(AmyHammerAttack == 1){
               if((Animation == "HAMMER_PUNCH" && AnimationFrame > 8) || (Angle > 45 && Angle < 310) || Action != ActionNormal ){
                     AmyHammerAttack = 0;
                     PlaySound(SFX.snd_object_bump, global.SFXVolume, 1.2, true);
                     // Create Hearts:
                        for(i = 0; i < 4; i+=1){
                     ...
                     if(KeyUp = true){
                        ...
                        AmyHammerAttack = 0;
                        Animation       = "HAMMER_AIR";
                        PlaySound(SFX.snd_character_jump, global.SFXVolume, 1.2, 1);
                        if(PhysicMode != "Underwater"){
                           if(KeySpecial){
                              YSpeed = -8
                           }else{
         ...
         // Stop Dash:
            if((Ground && AmyHammerAttack == 3) || Action != ActionNormal){
               if(KeyDown == true && AmyDashDirection = 2 && abs(XSpeed) < 8){
                  PlaySound(SFX.snd_character_roll, global.SFXVolume, 1.1, 1);
                  if(PhysicMode != "Underwater"){
